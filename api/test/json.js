...
describe('Test the various uses of the JSON field', function() {
  ...
  it('Should also work with record retreived via findOne', function(done) {
    User.create({
      username: 'Scott',
      jsonField: {
        likes: ['running', 'node']
    ...
    .then(function() {
      return User.findOne({where: {username: 'Scott'}})
    })
    .then(function(user) {
      user.jsonField.likes.push('chicken');
      return user.save();
    })
    .then(function(user) {
      expect(user.jsonField.likes).to.include('chicken');
      done();
    })
    .catch(done);
  });
